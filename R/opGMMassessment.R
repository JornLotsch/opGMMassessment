...
opGMMassessment <- function(Data, FitAlg = "MCMC", Criterion = "LR",
MaxModes = 8, MaxCores = 2048, PlotIt = FALSE, KS = FALSE, Seed) {
    ...
    switch(FitAlg, ClusterRGMM = {
      GMMfit <- lapply(list.of.Modes, function(x, Mixtures = Mixtures) {
        ...
        if (class(GMMfit_Mode) != "try-error") {
          Mixtures <- cbind(GMMfit_Mode$centroids,
          sqrt(GMMfit_Mode$covariance_matrices), GMMfit_Mode$weights)
        } else
          Mixtures <- Mixtures
      ...
      GMMfit <- parallel::mclapply(list.of.Modes, function(x, Mixtures = Mixtures) {
        ...
        if (class(GMMfit_Mode) != "try-error") {
          Mixtures <- cbind(GMMfit_Mode$Means,
          GMMfit_Mode$SDs, GMMfit_Mode$Weights)
        } else
          Mixtures <- Mixtures
        return(list(GMMfit_Mode, Mixtures))
      }, mc.cores = nProc)
      ...
      GMMfit <- parallel::mclapply(list.of.Modes, function(x, Mixtures = Mixtures) {
        ...
        if (class(GMMfit_Mode) != "try-error") {
          Mixtures <- cbind(GMMfit_Mode$mu,
          GMMfit_Mode$sigma, GMMfit_Mode$lambda)
        } else
          Mixtures <- Mixtures
        return(list(GMMfit_Mode, Mixtures))
      }, mc.cores = nProc)
